...
All pull requests targeting `main` are validated by GitHub Actions. The CI workflow installs the project in a clean Python 3.11 environment and runs the pytest suite. Contributions should keep CI green and add or update tests when behaviour changes.

CI does not require external LLM credentials or a production database. Tests for graph construction, entity resolution and GraphRAG retrieval should remain deterministic wherever possible.
